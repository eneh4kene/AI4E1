                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {PRODUCTS.map((product, index) => (
                        <ScrollReveal key={product.id} delay={index * 0.1}>
                            <motion.div
                                className="bg-white p-6 rounded-2xl border border-primary/5 hover:shadow-2xl hover:shadow-primary/10 transition-all flex flex-col h-full group"
                                whileHover={{ scale: 1.03, y: -5 }}
                                transition={{ duration: 0.2 }}
                            >
                                <div className="flex items-start justify-between mb-4">
                                    <div className="bg-primary/5 p-3 rounded-lg text-primary group-hover:bg-primary group-hover:text-primary-foreground transition-colors">
